# This is a Clear and important prompt
#   1. Remove the Generate Random Feedback Bottom
#   2. Remove the Quick Stats
#   3. Remove Feedback Over Time
#   4. Remove Template Information
#   4. In the filter by type: change [request other praise complaint] by the [Pricing,Appointments,Medical Staff,Customer Service,Emergency Services]
#   5. make the 8 objects in the list 10, Make it a list, that I can scroll through (As it is connected to a large database and we will apply queries to it)
...
def classify_feedback_simple(text):
    ...
    text_lower = text.lower()
    if any(word in text_lower for word in ["pricing", "appointment", "medical staff", "customer service", "emergency"]):
        return "Other"
    # Placeholder logic for sample text
...
def main():
    ...
    if st.session_state.records:
        ...
        sentiments = ['positive', 'neutral', 'negative', 'positive', 'negative']  # example 5 pie charts
        cols = st.columns(5)
